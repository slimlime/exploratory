...
(defun posterize-block (x y sx img out reduce-popcount)
  (declare (ignore reduce-popcount))
    ...
    (loop for j from y to (+ 7 y) do
	 (let ((fg (aref *c64-colours* (ash best-c -4)))
	   ...
	   (let ((best-pattern 0))
	     (unless (= fg bg)
	       (let ((best-diff most-positive-fixnum))
		 (loop for pattern across (if (= (mod j 2) 0)
					      *even-bytes*
					      *odd-bytes*) do
		      (let ((diff 0)
			    (bit #x80))
			(loop for i from x to (+ 7 x) do
			     (let ((img-col (aref img (+ i (* sx j)))))
			       (incf diff (colour-diff (if (= 0 (logand bit pattern))
							   bg
							   fg)
						       img-col)))
			     (setf bit (ash bit -1)))
	     ...
	     (setf (aref out (+ (* j (floor sx 8)) (floor x 8))) best-pattern))))    
    best-c))

;;TODO POPCOUNT REVERSAL
