...
(defun draw-line-code ()
  ...
  (with-namespace :draw-line2
    ...
    (LSR)
    (LSR)
    (TAY "Y now contains horizontal byte offset")
    (STA.ZP :D0)
    (LSR.ZP (hi-add :width))
    (LDA.ZP (lo-add :width))
    (ROR)
    ...
    (LSR)
    (CLC)
    (ADC.ZP :D0 "Add the byte width")
    (STA.AB :byte-x2 "Now we know where, horizontally, to stop")
    (dc ":ptr now contains the address of the screen byte containing our first pixel")
    (dc "x containts the bit number to start with")
    (LDA.AB "delta:0")
    ...
    (INY)
    (label+1 :byte-x2)
    (CPY 0) ;;if we went backwards, this would be redundant.
    (BEQ :end-bits)
    (JMP "bit:0")
    (label :end-bits)
